...
public class TeleporterParabolicPointer : MonoBehaviour
{
    [Tooltip("How steep teleport point surface can be. 0 allows any steepness, 1 requires perfect flatness")]
    [Range (0,1)]
    public float steepnessLimit = 0.7f;
    public bool freeTeleport;
    public bool singleTeleport;
    ...
    public bool ignoreSlopedSurfaces = true;
    ...
    private bool CalculateParabolicCurve(Vector3 p0, Vector3 v0, Vector3 a, float dist, int points, List<Vector3> outPts, out Vector3 normal)
    {
        ...
        for (int i = 0; i < points; i++)
        {
            ...
            Vector3 norm;
            bool endOnNavmesh;
            //bool cast = linecaster.Linecast(last, next, out endOnNavmesh, out castHit, out norm);
    ...
    private bool Linecast(Vector3 p1, Vector3 p2, out bool validPoint, out Vector3 hitPoint, out Vector3 normal)
    {
        ...
        {
            normal = hit.normal;
            if (Vector3.Dot(Vector3.up, hit.normal) < steepnessLimit && ignoreSlopedSurfaces)
            {
                validPoint = false;
                hitPoint = hit.point;
            ...
            if (freeTeleport || singleTeleport)
                validPoint = true;
            else
                validPoint = false;
